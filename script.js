/* Infinity Scripts — Protected Build */
;(function(_0xprot){
'use strict';
var _0xdom=['infinity-scripts.vercel.app','infscrip.my.id','localhost','127.0.0.1','vercel.app'];
var _0xcur=window.location.hostname;
var _0xpass=_0xdom.some(function(d){return _0xcur===d||_0xcur.indexOf(d)>-1;});
if(!_0xpass){document.documentElement.innerHTML='<h1 style="font-family:sans-serif;text-align:center;margin-top:20vh;color:#ff4444">⚠️ Unauthorized Access</h1>';return;}
var _0x223a=["c2NyaXB0cy5qc29u", "ZXZlbnQuanNvbg==", "Y2hhbmdlbG9nLmpzb24=", "YW5ub3VuY2VtZW50Lmpzb24=", "bmV3X3NjcmlwdF9ub3RpZi5qc29u", "aXNfZmF2b3JpdGVz", "aXNfdGhlbWU=", "cHdhX2Rpc21pc3NlZA==", "bG9jYWxTdG9yYWdl", "Z2V0RWxlbWVudEJ5SWQ=", "YWRkRXZlbnRMaXN0ZW5lcg==", "Y2xhc3NMaXN0", "aW5uZXJIVE1M", "dGV4dENvbnRlbnQ=", "ZmV0Y2g=", "dGhlbg==", "Y2F0Y2g=", "anNvbg==", "aGlkZGVu", "YWN0aXZl", "dmlzaWJsZQ=="];
function _0x6c37(i){
  try{return atob(_0x223a[i]);}catch(e){return _0x223a[i];}
}
'use strict';
let _0x23a4 = null;
...
if (!_0x23a4) return;
_0x23a4.prompt();
const { outcome } = await _0x23a4.userChoice;
if (outcome === 'accepted') {
...
let _0x2a1f = [];
let _0x3b2e = [];
let _0x4c3d = JSON.parse(localStorage.getItem('is_favorites') || '[]');
let _0x5d4e = 'all';
let _0x6e5f = '';
let _0x7f6a = 'default';
let _0x8a7b = 0;
...
_0x2a1f = await res.json();
} catch(e) {
_0x2a1f = [];
showToast('Gagal memuat scripts.json', 'error');
}
const statTotal = document.getElementById('stat-total');
if (statTotal) statTotal.textContent = _0x2a1f.length;
_0x3b2e = [..._0x2a1f];
renderTrending();
renderScripts();
...
const trending = _0x2a1f.filter(s => s.trending).slice(0, 6);
...
let result = [..._0x2a1f];
if (_0x5d4e !== 'all') result = result.filter(s => s.kategori === _0x5d4e);
if (_0x6e5f) {
const q = _0x6e5f.toLowerCase();
...
if (_0x7f6a === 'az') result.sort((a,b) => a.nama.localeCompare(b.nama, 'id'));
else if (_0x7f6a === 'za') result.sort((a,b) => b.nama.localeCompare(a.nama, 'id'));
else if (_0x7f6a === 'rating') result.sort((a,b) => (b.rating||0) - (a.rating||0));
else if (_0x7f6a === 'newest') result.sort((a,b) => (b.baru ? 1 : 0) - (a.baru ? 1 : 0));
_0x3b2e = result;
...
const count = cat === 'all' ? _0x2a1f.length : _0x2a1f.filter(s => s.kategori === cat).length;
...
const isFav = _0x4c3d.includes(script.id);
...
const nowFav = _0x4c3d.includes(script.id);
...
_0x4c3d.splice(idx, 1);
btn.classList.remove('active');
btn.innerHTML = '<i class="fa-regular fa-heart"></i>';
showToast('Dihapus dari favorit', 'info');
}
localStorage.setItem('is_favorites', JSON.stringify(_0x4c3d));
updateFavBadge();
...
if (!el) return;
if (_0x4c3d.length > 0) { el.textContent = _0x4c3d.length; el.classList.remove('hidden'); }
else { el.classList.add('hidden'); }
}
document.addEventListener('DOMContentLoaded', () => {
const favBtnTop = document.getElementById('fav-btn');
if (favBtnTop) favBtnTop.addEventListener('click', () => {
if (_0x4c3d.length === 0) { showToast('Belum ada script favorit', 'info'); return; }
_0x5d4e = 'all';
...
grid.innerHTML = '';
_0x2a1f.filter(s => _0x4c3d.includes(s.id)).forEach((s,i) => grid.appendChild(createCard(s,i)));
document.getElementById('trending-grid').innerHTML = '';
document.getElementById('empty-state').classList.add('hidden');
showToast(`${_0x4c3d.length} script favorit`, 'success');
scrollToScripts();
});
});
document.addEventListener('DOMContentLoaded', () => {
...
_0x6e5f = input.value.trim();
clear.classList.toggle('hidden', !_0x6e5f);
renderScripts();
});
...
if (prevBtn) prevBtn.addEventListener('click', () => goBanner((_0x8a7b - 1 + slides.length) % slides.length));
if (nextBtn) nextBtn.addEventListener('click', () => goBanner((_0x8a7b + 1) % slides.length));
...
? (_0x8a7b + 1) % slides.length
: (_0x8a7b - 1 + slides.length) % slides.length);
}, { passive: true });
}
_0x9b8c = setInterval(() => goBanner((_0x8a7b + 1) % slides.length), 5000);
}
...
let _0xdfc0    = new Date().getDay();
let currentAnimeGenre  = 'all';
let _0xf1e2   = '';
let animeLoaded        = false;
let animeView          = 'jadwal';
let _0x12f3  = null;
...
renderAnimeView();
if (_0x12f3) clearInterval(_0x12f3);
_0x12f3 = setInterval(updateCountdowns, 1000);
} catch(e) {
...
const today = animeWeekData[_0xdfc0] || [];
...
let list = [...(animeWeekData[_0xdfc0] || [])];
...
if (ttl) ttl.innerHTML = `<i class="fa-solid fa-calendar-day"></i> ${HARI[_0xdfc0]}`;
grid.innerHTML = '';
...
_0xf1e2 = e.target.value.trim();
if (aclear) aclear.classList.toggle('hidden', !_0xf1e2);
renderAnimeView();
});
if (aclear) aclear.addEventListener('click', () => {
qid('anime-search-input').value = '';
_0xf1e2 = ''; aclear.classList.add('hidden'); renderAnimeView();
});
...
btn.classList.toggle('active', d === _0xdfc0);
});
...
btn.classList.add('active');
_0xdfc0 = parseInt(btn.dataset.day);
updateAnimeStats();
renderAnimeView();
});
}
